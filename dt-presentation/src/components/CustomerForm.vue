<template>
  ...
  <div class="container col-12 col-md-8 bg-light p-3">
    ...
    <form class="p-5 fs-5 text bg-light" @submit.prevent="submitForm" ref="form" novalidate>
      
      <div v-if="errors.length" class="alert alert-danger" style="color: red; background-color: #f8d7da;">
...
export default {
  ...
  validateForm() {
    this.errors = [];
    if(!this.companyName)this.errors.push('Company name is required.');
    if(!this.companySize)this.errors.push('Company size is required.');
    if(!this.teamSize)this.errors.push('Number of developers is required.');
